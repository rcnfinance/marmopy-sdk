...
class IntentAction(object):
    def __init__(self, function):
        ...
        args_names = [
            var for var in function.__code__.co_varnames if var != "self"]
        ...
        self.abi['inputs'] = [{"name": arg, "type": arg_type}
                              for arg, arg_type in zip(args_names, args_types)]
    ...
    def __call__(self, instance, *args, **kwargs):
        ...
        self.abi['outputs'] = [
            {'name': '', 'type': self.function(instance, args)}]

        self.encoded = Contract._encode_abi(self.abi, args, data).decode()

        self.arguments = dict(zip([arg['name']
                                   for arg in self.abi['inputs']], args))
